...
type
  ...
  //Funkcje dodatkowe
    ...
    function  ReadMem(RHan : THandle; var Buffer; adr : Cardinal; size : Cardinal): TStatus;
    function  WriteMem(RHan : THandle; const Buffer; adr : Cardinal; Size : Cardinal): TStatus;
    ...
    function  ReadWord(RHan : THandle; adr : Cardinal;var  w : word):TStatus;
    function  ReadByte(RHan : THandle; adr : Cardinal;var  w : byte):TStatus;
...
begin
  ...
  try
    ...
    begin
      s := SL.Strings[0];
      if s = 'RCOM' then   DllHandle := ComDllHandle;
      if s = 'MCOM' then   DllHandle := ModbusComDllHandle;
...
function  TCmmDevice.ReadMem(RHan : THandle; var Buffer; adr : Cardinal; size : Cardinal): TStatus;
var
  _ReadMem   : TReadMem;
begin
  ProgressHandle := RHan;
...
function  TCmmDevice.ReadWord(RHan : THandle; adr : Cardinal;var  w : word):TStatus;
begin
  Result := ReadMem(RHan,w,adr,sizeof(w));
end;
function  TCmmDevice.ReadByte(RHan : THandle; adr : Cardinal;var  w : byte):TStatus;
begin
  Result := ReadMem(RHan,w,adr,sizeof(w));
end;

function  TCmmDevice.WriteMem(RHan : THandle; const Buffer; adr : Cardinal; Size : Cardinal): TStatus;
var
  _WriteMem  : TWriteMem;
begin
  ProgressHandle := RHan;
...
begin
  Result := WriteMem(RHan,w,adr,sizeof(w));
end;
