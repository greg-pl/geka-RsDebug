...
begin
  ...
  begin
    ...
    begin
      ...
      st := Dev.WriteMem(Handle,MemBuf[0],PhAdr,GlMemSize);
      DoMsg('WriteMem='+Dev.GetErrStr(st));
    end
    else
...
begin
  ...
  begin
    PhAdr := AreaDefItem.GetPhAdr(GlMemAdr);
    TT:=GetTickCount;
    st:=Dev.ReadMem(Handle,MemBuf[0],PhAdr,GlMemSize);
    TT:=GetTickCount-TT;
    if st=stOK then
    begin
      if TT<>0 then
        DoMsg(Format('RdMem v=%.2f[kB/sek]',[(GlMemSize/1024)/(TT/1000.0)]))
      else
        DoMsg('RdMem OK');
    end
    else
      DoMsg('ReadMem='+Dev.GetErrStr(st));
    GlBufRdy:=true;
    FillWykres;
  end
  else
