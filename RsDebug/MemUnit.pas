...
begin
  ...
  begin
    //MemFrame.ClrData;
    TT := GetTickCount;
    Result:=Dev.ReadMem(Handle,MemFrame.MemBuf[0],Adr,Size);
    TT := GetTickCount-TT;
    if Result=stOK then
    begin
      if TT<>0 then
...
begin
  GetFromText(Adr,ShowAdr,Size,RegSize);
  Result:=Dev.WriteMem(Handle,MemFrame.MemBuf[0],Adr,Size);
  DoMsg(Dev.GetErrStr(Result));
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          st := Dev.WriteMem(Handle,MemFrame.MemBuf[i],Adr,n);
          DoMsg(Format('WriteMem, adr=0x%X, size=%u  :%s',[Adr,n,Dev.GetErrStr(st)]));
