...
function  TPictureViewForm.ReadPtrValue(A : cardinal): cardinal;
var
  Size : integer;
...
function TPictureViewForm.GetAddr : cardinal;
begin
  Result := MapParser.StrToAdr(AdresBox.Text);
  if AdrModeGroup.ItemIndex=1 then
  begin
    Result  := AreaDefItem.GetPhAdr(Result);
    Result := ReadPtrValue(Result);
  end;
  Result  := AreaDefItem.GetPhAdr(Result);
end;
...
begin
  ...
  st:=Dev.ReadMem(Handle,PicBuffer[0],Adr,MemSize);
  FillBitmap(PicBuffer);
  DrukImage.Picture.Assign(bmp);
  DoMsg(Format('RdPic=%s',[Dev.GetErrStr(st)]));
  Sm := TMemoryStream.Create;
  SM.Write(PicBuffer[0],MemSize);
...
begin
  ...
  Adr := GetAddr;
  st:=Dev.WriteMem(Handle,PicBuffer[0],Adr,MemSize);
  DoMsg(Format('WrPic=%s',[Dev.GetErrStr(st)]))
end;
