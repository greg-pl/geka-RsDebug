...
procedure GetHostByNameGK(HostName : string; SL :TStrings);
type
...
function GetHostIpByName(HostName : string): cardinal;
type
  TaPInAddr = Array[0..250] of PInAddr;
  PaPInAddr = ^TaPInAddr;
var
...
begin
  ...
  begin
    ...
    while PAdrPtr^[i] <> nil do
    begin
      result := cardinal(PAdrPtr^[i].S_addr);
...
procedure GetLocalAdresses(SL :TStrings);
begin
  GetHostByNameGK(GetHostName,SL);
end;
...
function  GetHostAdress(HostName: string) : string;
var
  SL :TStringList;
begin
  Result := '';
...
begin
  if Ip<>'' then
  begin
    ipD := cardinal(inet_addr(pchar(ip)));
    if ipD=cardinal(INADDR_NONE) then
    begin
      ipD := GetHostIpByName(Ip);
    end;
